...
$query = "SELECT * FROM admin WHERE User = '$admin_username'";
$result = $conn->query($query);

if ($result->num_rows > 0) {
    $row = $result->fetch_assoc();
    $admin_contact = $row['Contact_Number'];
    $admin_name = $row['User']; // Fetch admin's name
} else {
...
<body>
    <div class="container">
        ...
        <div class="navigation">
            ...
            <div class="navigation-item" onclick="openPopup('list')">List</div>
    ...
    ?>
        ...
        <div class="admin-actions">
            ...
            <button onclick="toggleUploadPopup()">Upload Content and Post Announcements</button>
        </div>
    </div>

    <div id="upload-popup">
        <form method="post" enctype="multipart/form-data">
            <label for="file-upload-poster">Upload Poster:</label>
            <input type="file" id="file-upload-poster" name="file-upload-poster" onchange="previewFile('file-upload-poster')">
            <br>
            <span id="poster-file-details" class="hidden"></span>

            <label for="file-upload-announcement">Upload Announcement:</label>
            <input type="file" id="file-upload-announcement" name="file-upload-announcement" onchange="previewFile('file-upload-announcement')">
            <br>
            <span id="announcement-file-details" class="hidden"></span>

            <button type="submit">Upload</button>
            <button onclick="openGallery('announcement')">View Announcement Gallery</button>
            <button onclick="openGallery('poster')">View Poster Gallery</button>
        </form>
    </div>
...
function openPopup(page) {
    if (page === 'list') {
        window.open('list.php', '_blank', 'width=600,height=400');
    } else {
